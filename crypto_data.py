...
class FetchDataYahoo:  # Create FetchDataCryptoCompare for data per seconds
...
class BasicStats(FetchDataYahoo):
    ...
    def annualized_sharpe_ratio(self):
        ...
        return ann_sharpe(lambda x: x*(x > 0))
    ...
    def SMA(self, criteria='Close', slow=50, fast=20):  # the criteria can also be 'Volume'
        ...
        if type(self.data[criteria]) == pd.Series:  # we need to do it differently when only one crypto is analyzed
            self.data[str(criteria+'_slowSMA')] = slow_table
            self.data[str(criteria + '_fastSMA')] = fast_table
            return pd.concat([self.data[str(criteria + '_slowSMA')], self.data[str(criteria + '_fastSMA')]],
                             join='inner', axis=1)
    ...
    def EMA(self, criteria='Close', slow=14, fast=3):  # criteria can be 'Volume' also
        ...
        if type(self.data[criteria]) == pd.Series:  # we need to do it differently when only one crypto is analyzed
            self.data[str(criteria+'_slowEMA')] = slow_table
            self.data[str(criteria + '_fastEMA')] = fast_table
            return pd.concat([self.data[str(criteria + '_slowEMA')], self.data[str(criteria + '_fastEMA')]],
                             join='inner', axis=1)
